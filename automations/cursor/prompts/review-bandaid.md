...
description: >-
  ...
  either way. Never performative agreement.
trigger: pullRequestReviewComment
contains: "@review-bandaid"
...
If `SYNAPSE.md` is absent, unparseable, or missing any of Identity, Secrets, Verify, or Protected, STOP at GATE 0 as UNVERIFIABLE and name the missing section.

Everywhere below, `$REPO`, `$BASE`, `$PR_SECRET`, the verify commands, and the protected list come from `SYNAPSE.md`.

TRIGGER

One inline pull-request review thread was sent to this automation.

Read the available identifiers from <automation_trigger_info>: pullRequestNumber, pullRequestUrl, reviewThreadId, reviewCommentId, reviewCommentUrl. Call the PR number $PR.
...
All GitHub PR metadata, review-thread, reply, and resolution operations must use the secret named in `SYNAPSE.md` under Secrets.pull-requests. Scope it per command:
...
If the secret is missing, authentication fails, or it lacks the required PR permissions, STOP.

AUTHENTICATION SPLIT

Use the PR secret for: `gh pr view`, `gh api`, `gh api graphql`, PR metadata reads, review-thread reads, inline review replies, review-thread resolution, thread-state confirmation.
...
Every GraphQL command must use `GH_TOKEN="$PR_SECRET" gh api graphql <arguments>`.

Match reviewThreadId, reviewCommentId, or the `discussion_r...` identifier from the trigger URL to exactly one thread.
...
```
GH_TOKEN="$PR_SECRET" gh api --method POST \
...
```
GH_TOKEN="$PR_SECRET" gh api graphql -f query='
...
After resolving, re-read the target thread with one PAT-authenticated GraphQL call and confirm `isResolved` is true.

AUTH FAILURE RULES

If a PR or thread read fails: do not inspect flat comments as a substitute, do not edit code, do not reply or resolve, do not use another credential. STOP.
...
2. Resolve $PR and the target thread from <automation_trigger_info>.
...
5. Fetch the complete target thread through PAT-authenticated GraphQL.
...
1. Fetch the exact PR head branch using normal git credentials.
2. Confirm the fetched head equals ORIGINAL_PR_HEAD.
3. Create a temporary linked worktree and local review-bandaid branch based exactly on ORIGINAL_PR_HEAD.
4. Perform all inspection and edits inside that worktree.
5. Confirm the worktree is clean.

Do not edit the main checkout, work directly on `$BASE`, merge or rebase `$BASE` into the PR branch, reuse an unrelated worktree, or pass GH_TOKEN to git commands.
...
```
GH_TOKEN="$PR_SECRET" gh pr view "$PR" --repo "$REPO" --json headRefOid
```

Also re-read the target thread using PAT-authenticated GraphQL.
...
The worktree must remain clean. No file may have changed. Do not commit or push.
...
After the reply succeeds: resolve THREAD_ID, read the thread once and confirm `isResolved` is true, remove the temporary worktree and branch, terminate.

If the reply fails, do not resolve. If resolution fails, leave the thread open. Do not post a top-level fallback. Do not use another credential.
...
After replying: confirm through PAT-authenticated GraphQL that the thread remains unresolved, remove the temporary worktree and branch, terminate.
...
3. Re-read the target thread using PAT-authenticated GraphQL.
...
Create exactly one focused commit. Confirm ORIGINAL_PR_HEAD is an ancestor, exactly one new commit was created, and the worktree is clean after committing.
...
If the push fails, do not reply or resolve. If the reply fails, do not resolve. If resolution fails, leave the thread open. Do not post a top-level fallback, use another credential, open another PR, or merge the PR.
...
If the reply fails, still leave the thread unresolved. Do not post a top-level fallback. Do not use another credential.
