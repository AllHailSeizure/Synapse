...
description: >-
  ...
  semantic. Runs unattended in GitHub Actions.
...
An open GitHub pull request is reported as conflicting with `$BASE`. The PR number is passed to this skill as its argument. Call it $PR.
...
If authentication fails or lacks the required PR permissions, STOP. Do not attempt interactive authentication.
...
Use `gh` for: `gh pr view`, `gh pr comment`, `gh api`, PR metadata reads, PR mergeability reads, PR comments. Use plain `git` for `git fetch` and `git push`.
...
5. Read the target PR mergeability.
6. Comment the outcome on the target PR.
...
Read committed repository instructions before changing anything: AGENTS.md, CLAUDE.md, applicable rule files under `.cursor/rules/`, and relevant git/worktree sections of CONTRIBUTING.md.
...
5. Check out a temporary local branch based exactly on ORIGINAL_PR_HEAD.
6. Confirm the tree is clean.

The checkout is an ephemeral clone created for this run, so no further isolation is needed. Do not work directly on local `$BASE`.

Gate passes only if the branch starts exactly at ORIGINAL_PR_HEAD, BASE_HEAD is recorded, the tree is clean, and no source file has been edited. Otherwise STOP.

GATE 2 — EXACTLY ONE MERGE ATTEMPT

Run one merge without committing and without automatic remembered resolutions:
...
```
gh pr view "$PR" --repo "$REPO" --json headRefOid,mergeable,mergeStateStatus
```
...
3. Discard only this automation's uncommitted changes.
...
6. If commenting is unavailable, record the failure only in the run log.
7. Terminate.
...
3. If commenting fails, record it in the run log.
4. Terminate.
